...
class GraspPoseEstimatorMCC:
    ...
    def predict_grasp_point(self, img, xyz, kpts):
        """
        Args:
        ...
        self.mcc_reconstructor.set_model_input(img, seg, xyz)
        pred_occupy, pred_colors, occupy_threshold, verts, faces, vert_normals, values = self.mcc_reconstructor.reconstruct()
        self._log("Generating potential grasp points...")
        verts_1_new, verts_2_new, _, _, _ = self._generate_potential_grasp_points(verts, vert_normals, occupy_threshold=occupy_threshold, scale=1/self.mcc_reconstructor.masked_max_dist)
    ...
    def _compute_grasp_point(self, contact_verts_1: np.ndarray, contact_verts_2: np.ndarray):
        """
        Args:
        ...
        grasp_point = grasp_points[np.argmin(np.linalg.norm(grasp_points - moving_obj_center, axis=1))]
        return grasp_point
    ...
    def _check_collision_v2(
        ...
        for i in range(n_contact_points):
            sample_points[i*n_sample_points:(i+1)*n_sample_points] = sample_xz_plane_grid(contact_verts_unnormalized[i] + gripper_offset, gripper_pad_width, n_points_per_dim)

        sample_points = self.mcc_reconstructor.normalize(sample_points)
        
        # query model
        pred_occupy, _ = self.mcc_reconstructor.query_model(torch.from_numpy(sample_points))
    ...
    def _check_collision(
        ...
        contact_verts_1_unnormalized = self.mcc_reconstructor.denormalize(contact_verts_1)
        contact_verts_2_unnormalized = self.mcc_reconstructor.denormalize(contact_verts_2)
    ...
    def _generate_potential_grasp_points(self, verts: np.ndarray, vert_normals: np.ndarray, occupy_threshold: float, scale: float):
        """
        Args:
            verts: (N, 3)
            vert_normals: (N, 3)
        ...
        has_collision_1, _, _ = self._check_collision_v2(verts_1, occupy_threshold=occupy_threshold, site="left")
        has_collision_2, _, _ = self._check_collision_v2(verts_2, occupy_threshold=occupy_threshold, site="right")
